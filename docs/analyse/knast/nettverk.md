...
Det er også stengt av mot internett som default. Nada tilbyr en liste med åpninger som de aller fleste har behov for. Utover den må du legge til URL-er du vil snakke med selv ved å trykke på "konfigurer" ved siden av "Tidsbegrensede internettåpninger" på hovedsiden, eller under "Instillinger->Tidsbegrensede åpninger". URLene må oppgis med [syntax beskrevet her](https://cloud.google.com/secure-web-proxy/docs/url-list-syntax-reference). Det er god praksis å åpne for minst mulig, for eksempel er `github.com/navikt/*`  bedre enn `github.com/*`.
Det er ikke alltid åpenbart hvilke URLer forskjellige klienter og extensions prøver å snakke med. Hvis du opplever at noe ikke fungerer som forventet, sjekk ut "Blokkerte URLer" under "URL-håndtering". Der vil du se URLer som har blitt blokkert og kanskje må legges til i allowlisten din. Husk å ha et bevisst forhold til hva du åpner for. 
